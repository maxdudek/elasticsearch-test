...
if (len(sys.argv) < 2):
    print("ERROR: No directory specified. Run using 'python ingest.py <config-directory>'")
    exit(1)
...
# If true, delete and recreate the index before ingesting
DELETE_INDEX = INGEST_CONFIG['delete_index']
...
# The function in this script which will be used to transform the documents
...
def bulkIngest(es, dataPath):
    ...
    # Bulk ingest
    print('\nBeginning bulk ingest on index ' + INDEX + '...')
    startTime = time.time()
    # See https://elasticsearch-py.readthedocs.io/en/master/helpers.html 
