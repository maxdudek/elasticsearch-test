...
def bulkIngest(es, dataPath):
    ...
    startTime = time.time()
    try:
        printJson(helpers.bulk(es, bsonIter(dataFiles, bulk=True)))
    except BulkIndexError as e:
        print("Writing errors...")
        writeJson(e.errors, 'bulk_errors.json')
    timeTaken = time.time() - startTime
    print('Bulk ingest time: ' + str(timeTaken / 60.0) + ' minutes')
    time.sleep(1)
    count = es.count(index=INDEX)
    numDocs = count['count']
    print('Ingested %d documents at a rate of %f documents per second' % (numDocs, numDocs / timeTaken))
...
# writeJson(es.indices.get_settings(index=INDEX, include_defaults=True)[INDEX], 'index_settings.json')
# writeJson(es.cluster.get_settings(include_defaults=True), 'cluster_settings.json')
